...
Use `headers` for any HTTP MCP server that requires authentication. LiteLLM-compatible servers usually expect the key in the `Authorization` header with the `Bearer ` prefix.
